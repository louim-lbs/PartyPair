...
* C'est aussi ce service que declenche l'automatisation.
  ...
  * un reveil declenche trop tot trouverait le Bluetooth encore eteint.
  ...
  ```
   ...
   private val PROMPTING_ACTIONS =
       setOf(ACTION_START, ACTION_TOGGLE)
   ...
   suspend fun refreshState(context: Context) {
       ...
       val on = runCatching {
           PartyController(context).isAlreadyOn()
       }.getOrDefault(false)
   ...
   fun start(
       ...
       when (action) {
           ACTION_POWER_OFF -> {
               state.value = UiState(Step.FADING_OUT)
           }
